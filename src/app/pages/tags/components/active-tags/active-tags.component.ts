...
export class ActiveTagsComponent implements OnInit {
  ...
  @ViewChild('sTax', { static: true }) adsSearchInput: ElementRef;
  ngOnInit() {
    fromEvent(this.adsSearchInput.nativeElement, 'keyup')
      .pipe(
        map((event: any) => {
  ...
  getAllTags(event?, sortName?, text?) {
    const list = {
    ...
    this.service.tagsList(list, event).subscribe(
      (res: any) => {
