...
@Component({
  selector: 'app-tags-userdetails',
  templateUrl: './tags-userdetails.component.html',
  styleUrls: ['./tags-userdetails.component.scss'],
})
export class TagsUserdetailsComponent implements OnInit {
  constructor(
  ...
  getAssetList() {
    ...
    this.service.assetGetList(list).subscribe(
      (res: any) => {
        ...
        // })
        this.userDetails = res.result.users;
        this.listGroup = res.result;
        if (res.result.images && res.result.images.length > 0) {
          this.carouselTileItems = res.result.images;
          this.carouselTileItems[0].active = true;
          this.lazyLoad = this.carouselTileItems[0].url;
        } else {
          this.listGroup.details.map(x => {
            x.attributes.map(y => {
              if (y.typeCode === 13) {
                ...
                // console.log('images',  y.value_13)
              }
            });
          });
        }

        const breadcrumb = { uTitle: this.listGroup.title };
  ...
  carousalIndex(index: number) {
    ...
    this.lazyLoad = this.carouselTileItems[index].url;
