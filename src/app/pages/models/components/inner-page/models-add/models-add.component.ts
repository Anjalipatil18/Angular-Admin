...
export class ModelsAddComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // console.log('modelsData submitted------>', this.service.getModelsData());
    this.route.params.subscribe(params => {
      console.log('params---------------------------->', params);
      this.statusCode = params.sCode;
      this.assetId = params.id;
      this.statusText = params.statusText;
      const title = this.assetId === '1' ? 'Add' : 'Edit';
    ...
    const val = this.assetId === '1' ? this.getAllModels() : '';
  }
    ...
    this.service.languageActive().subscribe(
      (res: any) => {
        ...
        if (index > -1) {
          ...
          this.addForm.controls[
            'brands_' + this.langSelectors[index].languageCode
          ].setValidators(Validators.required);
          this.addForm.controls[
            'brands_' + this.langSelectors[index].languageCode
          ].updateValueAndValidity();
  ...
  reportCheckbox(event, i, lang) {
    ...
    if (event.target.checked) {
      ...
      this.addForm.controls['brands_' + lang].setValidators(
        Validators.required
      );
  ...
  validation() {
    this.langSelectors.map(x => {
      ...
      const valuea = this.addForm.value['valuea_' + [x.languageCode]];
  ...
  getUsers(data: any) {
    ...
    this.langSelectors.map(x => {
      if (x.active) {
        this.brandsEmpty = false;
        console.log(data[`brands_${x.languageCode}`]);
        if (!data[`brands_${x.languageCode}`]) {
          this.brandsEmpty = true;
        } else {
          this.brandsEmpty = false;
          console.log('a');
        }
    ...
    console.log(this.brandsEmpty);
    if (this.brandsEmpty) {
      this.showToast('top-right', 'brands' + ' fields are missing!', 'danger');
      return;
    } else if (!data.tag) {
      this.showToast('top-right', 'tag' + ' fields are missing!', 'danger');
      return;
    }
    let list;
    if (this.assetId === '1' && data.brands_en !== '') {
      list = {
        brandAndModels: data,
        // models: data.models_en,
        // type: data.tag_en
      };
    } else if (this.assetId !== '1' && data.brands_en !== '') {
      list = {
        id: this.assetId,
        brandAndModels: data,
      };
    ...
    console.log('before post api--------> ', this.assetId, list);
    this.service.assetPostList(list, this.assetId).subscribe(
      (res: any) => {
        // this.location.back();
        console.log('res brands', res);
        // this.location.go('pages/models');
        this.router.navigate(['pages/models']);
        this.showToast('top-right', 'Brands added successfully!', 'success');
      },
      error => {
        console.log(error);
  ...
  getAllModels(event?) {
    ...
    this.service.modelsList(list, event).subscribe(
      (res: any) => {
        ...
        console.log(res.data[0].languages, 'res.data[0].languages');
        ...
        for (const prop in resEdit) {
          if (resEdit[prop]) {
            console.log(
              'this.langSelectors123456',
              this.langSelectors,
              resEdit[prop]
            );
            console.log(resEdit[prop]);
            const index = this.langSelectors.findIndex(
              x => x.languageCode === resEdit[prop]
            );
        ...
        // console.log('zzzzzzzzzzzzzzzz',this.allBrands);
        const models = [];
        this.allBrands.map(x => {
          // x.models.map(y => {
          //   models.push(y.modelName);
          // })
        });

        // this.addForm.controls['tag'].setValue(this.allBrands[0]['tag']);

        this.langSelectors.map(x => {
          console.log('aaaaaaaaaaaaaaaaaaaa', x);
          if (x.active) {
            // console.log(this.allBrands);
            this.addForm.controls['brands_' + [x.languageCode]].setValue(
              this.allBrands[0]['brands_' + [x.languageCode]]
            );

            this.addForm.controls.tag.setValue(this.allBrands[0].tag);
            // this.allBrands.map(x => {
            //   console.log('x', x);
            //   x.models+'_'+[x.languageCode].map(y => {
            //     typeof y == 'object' ? '' : models.push(y);
            //   })
            // })
            this.items = this.allBrands[0]['models_' + [x.languageCode]];
            this.addForm.controls['models_' + [x.languageCode]].setValue(
              this.allBrands[0]['models_' + [x.languageCode]]
            );

            // let index = this.items.findIndex(arr => arr.lang === [x.languageCode]);
            // index > -1 ? this.items[index].value.push(event.value) : this.items.push({ lang: [x.languageCode], value: this.allBrands[0]['models_'+[x.languageCode]]});
            // this.items.map(x => {
            //   x.lang == [x.languageCode] ? this.addForm.controls['models_'+[x.languageCode]].setValue(x.value) : this.addForm.controls['models_'+[x.languageCode]].setValue('');
            // })
          }
        });
      },
